...
/**
 * Parses strings to numbers quickly. Arguments not checked or trimmed. Limited formats accepted.
 * 
 */
public class QuickParseUtil {

	/**
	 * Parse string containing a float. Assumes input trimmed, not null, not infinity, not NaN, not using exponent
	 * representation. Handles only: [-+]?\d+(.\d+)?
	 * <p>
	 * Current implementation took 12ms to parse 1000 test strings, Float.parseFloat took 465ms to parse the same
	 * strings.
	 * 
	 * @param aInput
